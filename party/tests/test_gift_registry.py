...
def test_gift_registray_page_lists_gifts_for_a_party_by_id(authenticated_client, create_user, create_party, create_gift):
...
@pytest.mark.django_db
def test_partial_gift_update_updates_gift_and_returns_its_details_including_party(
authenticated_client, create_user, create_party, create_gift
):
    party = create_party(organizer=create_user, venue="Best venue")
    gift = create_gift(party=party)
